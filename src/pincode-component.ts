            ...
            <div class="pincode-input">
                <ion-grid class="pincode-input-grid">
                    <ion-row class="pincode-input-row">
                        <ion-col *ngFor="let item of codeArr; let i = index"><span
                                [class.on]="isNum(codeArr[i])"></span></ion-col>
                    </ion-row>
                </ion-grid>
            </div>
    ...
    numClick(num: number) {
        ...
        if (emptyIndex < this.maxLen - 1) {
            ...
            if (this.d.pinHandler) {
                // If we have a PIN handler, call it with the completed PIN.
                if (this.d.pinHandler(this.getValues())) {
                    this.dismiss('done');
                } else {
                    // Clear the PIN and don't dismiss.
                    this.restoreClick();
                }
            } else {
                this.dismiss('done');
